...
const double _kWuRowHeight  = _kTabH;
const double _kActionHeight = _kTabH;
const double _kActionGap    = _kTabGap;
...
const List<double> kStandardWeights = [
  0, 1, 1.5, 2, 2.5, 3, 4, 5, 6, 7.5, 8, 10, 12, 12.5,
  15, 17.5, 20, 22.5, 25, 27.5, 30, 32.5, 35, 37.5, 40,
  42.5, 45, 47.5, 50, 55, 60, 65, 70, 75, 80, 85, 90,
...
class _ActiveWorkoutScreenState extends State<ActiveWorkoutScreen>
  ...
  Future<void> _loadPrefs() async {
    ...
    setState(() {
      for (int i = 0; i < widget.routine.exercises.length; i++) {
        ...
        if (lw != null) {
          int best = 0; double bd = double.infinity;
  ...
  void _startRestTimer() {
    ...
    setState(() {
      _restActive      = true;
      _restSecondsLeft = _kDefaultRestSeconds;
    });
  ...
  void _cancelRestTimer() {
    _restTimer?.cancel();
    setState(() { _restActive = false; _restSecondsLeft = 0; });
  }
  ...
  void _scrollSidebarToCenter(int exIdx, int setIdx) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final hasPrev = pos > 0;
      if (!(hasNext && hasPrev)) {
        if (!hasPrev) ctrl.animateTo(0,
            duration: const Duration(milliseconds: 280), curve: Curves.easeOut);
        if (!hasNext) ctrl.animateTo(ctrl.position.maxScrollExtent,
            duration: const Duration(milliseconds: 280), curve: Curves.easeOut);
        return;
      }
  ...
  // Long-press remove with undo snackbar
  ...
  void _toggleSet(int exIdx, int setIdx) {
    ...
    setState(() {
      ...
      if (sd.isCompleted) {
        ...
        final next = _setsData[exIdx]
            .indexWhere((s) => !s.isCompleted, setIdx + 1);
        if (next != -1) {
          _activeSetIndex[exIdx] = next;
          _scrollSidebarToCenter(exIdx, next);
        } else {
          // All sets in this exercise done — nudge > button
          _triggerNudge();
        }
      } else {
        // Undoing a set — cancel rest timer
        _cancelRestTimer();
      }
    });
  }
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${_exerciseIndex + 1} of ${exercises.length} exercises',
                        style: GoogleFonts.dmSans(
                          fontSize: 10,
                ...
                // Next / Finish — with nudge animation
                GestureDetector(
                  onTap: !isLast
                      ? () => _navigateToExercise(_exerciseIndex + 1)
                      : _allDone
        ...
        Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(
                height: _kWuRowHeight,
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.stretch,
                  children: [
                    // Only show WU tab if this exercise has a warm-up set
                    if (setsData.any((s) => s.isWarmUp))
                      _buildWuTab(setsData, activeSetIndex, onSelectSet),
                    if (setsData.any((s) => s.isWarmUp))
                      const SizedBox(width: 14),
                    // If no WU, add a SizedBox of same width to keep layout
                    if (!setsData.any((s) => s.isWarmUp))
                      SizedBox(width: _kTabW + 14),
                    Expanded(
                      child: Row(
                      ...
                      child: GestureDetector(
                        ...
                        child: AnimatedContainer(
                          ...
                          child: Row(
                            ...
                            children: activeSet.isCompleted
                                ? [
              ...
              if (showTick)
                Positioned(
                  ...
                  child: Container(
                    width: 11, height: 11,
                    decoration: BoxDecoration(
                        color: tickColor, shape: BoxShape.circle),
                    child: CustomPaint(
...
class _PlateCalculatorSheetState extends State<_PlateCalculatorSheet> {
  ...
  @override
  void initState() { super.initState(); _barWeight = 20.0; }
  ...
  Widget build(BuildContext context) {
    ...
    final actual = _barWeight +
        plates.fold<double>(0, (s, e) => s + e.key * e.value * 2);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(children: kBarWeights.map((bw) {
          ...
          if (plates.isEmpty)
            ...
            Wrap(
              ...
              children: plates.map((e) => Container(
          ...
          children: [
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: Center(child: Text('BACK TO PLAN',
                    style: GoogleFonts.dmSans(
